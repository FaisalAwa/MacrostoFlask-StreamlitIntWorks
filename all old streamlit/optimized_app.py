...
def add_question_numbers(doc):
    """Add sequential numbering to Question: tags"""
    counter = 1
    for para in doc.paragraphs:
...
def process_docx_file(uploaded_file, add_numbers=False):
    """Process document with all macros applied"""
    macro_status = {}
    ...
    macro_status["Combined Text Operations"] = {"status": "Applied", "time": f"{(time.time() - start_time):.2f}s"}
...
with st.expander("📘 View Document Processing Steps"):
    st.text(
        ...
        "5. (Optional) Add sequential numbering to all 'Question:' tags (e.g., Question: 1, 2, ...).\n"
        ...
        "8. Save and return the processed .docx file for download.\n\n"
...
if uploaded_file:
    ...
    if st.button("Apply Macros"):
        ...
        # Step 5: Add Question Numbers (optional)
        if add_numbers:
        ...
        # Step 6: Add Explanation Tags (optimized)
        ...
        # Step 7: Add Line Spacing (optimized)
